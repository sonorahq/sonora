...
use state::{Library, LibraryPart, LibraryState, Origin, Playback, PlaylistRow};
...
impl PlaylistSource {
    ...
    fn state<'a>(&self, cx: &'a App) -> &'a LibraryState {
    ...
    fn outline<'a>(&self, cx: &'a App) -> &'a [PlaylistRow] {
        match self.state(cx) {
            LibraryState::Ready { outline, .. } => outline.as_slice(),
            _ => &[],
        }
    }
