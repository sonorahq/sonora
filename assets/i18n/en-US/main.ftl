...
settings-output-latency = Output delay
settings-output-latency-detail = How far behind the app your speakers are. The seek bar and the lyrics are pulled back by it, so both follow what you hear.
settings-output-latency-value = { $seconds } s
settings-airplay-latency = Detect AirPlay
settings-airplay-latency-detail = Uses AirPlay's own two-second delay while it is the output, in place of the one set above
...
settings-group-latency = Latency
