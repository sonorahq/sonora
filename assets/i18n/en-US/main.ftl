...
login-sign-in-google = Sign in with Google
login-manual-sign-in = Use manual sign-in
login-connect-cookies = Use manual sign-in
login-cookie-open = Open YouTube Music
login-cookie-submit = Continue
login-manual-title = Use a manual YouTube Music sign-in
login-manual-hint = Paste the request header here
login-cookie-title = Use a manual YouTube Music sign-in
login-cookie-hint = Paste the request header here
...
login-cookie-step-note = Paste the entire value below. It must include SAPISID and __Secure-3PAPISID.
